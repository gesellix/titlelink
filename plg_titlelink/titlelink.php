...
class plgContentTitleLink
{
  ...
  var $dir        = "mambots/content/titlelink_plugins";
   ...
   */
  function plgContentTitleLink()
  {
    global $database, $mosConfig_live_site, $_MAMBOTS, $titlelink_cache;

    // check if param query has previously been processed
    if ( !isset($_MAMBOTS->_content_mambot_params['titlelink']) )
    {
      // load mambot params info
      $query = "SELECT params FROM #__mambots WHERE element = 'titlelink' AND folder = 'content'";
      $database->setQuery( $query );
      $database->loadObject( $mambot );

      // save query to class variable
      $_MAMBOTS->_content_mambot_params['titlelink'] = $mambot;
    }

    // pull query data from class variable
    $mambot = $_MAMBOTS->_content_mambot_params['titlelink'];
    $param = new mosParameters( $mambot->params );

    $this->trigger_prefix = $param->get( 'trigger_prefix', "{ln" );
    $this->trigger_suffix = $param->get( 'trigger_suffix', "}" );
    $this->separator = $param->def( 'separator', ":" );
    $this->enablenewcontent = $param->def( 'enablenewcontent', 0 );
    $this->disp_link = $param->def( 'disp_link', 1 );
    $this->disp_tooltip = $param->def( 'disp_tooltip', 1 );
  ...
  function onPrepareContent(&$article, &$params, $page = 0)
  {
        ...
        foreach ($fmatch as $match)
        {
          ...
          if (empty($link))  // did the search functions already set a link?
          {
            ...
            if (!array_key_exists($phrase, $titlelink_cache))
            {
              ...
              $result = $this->getByPlugins($this->plugin_cache, $phrase, false);
              if ((count($result) != 2) && ($partial_match))
              {
                // no exact match found --> try partial match
                $result = $this->getByPlugins($this->plugin_cache, $phrase, true);
              }
            }
            else
            {
    ...
    $plugin_to_call = mosGetParam($_GET, $this->plugin_call, null, _MOS_ALLOWRAW);
...
// internal functions
  ...
  function getByPlugins($plugins, $phrase, $partial_match)
  {
    $count = count($plugins);
    for ($i = 0; $i < $count; $i++)
    {
      $result = call_user_func($plugins[$i], $phrase, $partial_match);
  ...
  function getPluginFunctions($dir, $pluginmask)
  {
    $files = null;
    $dh  = opendir($dir);
...
$_MAMBOTS->registerFunction('onPrepareContent', 'botTitleLink');

// check, if already loaded. it could have been loaded by an external call
if (!function_exists('botTitleLink'))
{
  function botTitleLink($published, &$row, &$params, $page = 0)
  {
    if (!$published)
    {
      return;
    ...
    $titlelinkCls =& new plgContentTitleLink();

    return $titlelinkCls->onPrepareContent($row, $params, $page);
  }
} // end of if (!function_exists('botTitleLink'))
